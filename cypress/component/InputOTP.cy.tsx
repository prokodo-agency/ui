import InputOTPClient from "../../src/components/inputOTP/InputOTP.client"

describe("InputOTP", () => {
  it("renders 6 inputs by default", () => {
    cy.mount(<InputOTPClient groupLabel="One-time password" />)
    cy.get("input").should("have.length", 6)
  })

  it("renders with a group label", () => {
    cy.mount(<InputOTPClient groupLabel="Verification code" />)
    cy.contains("Verification code").should("be.visible")
  })

  it("renders custom length inputs", () => {
    cy.mount(<InputOTPClient groupLabel="PIN" length={4} />)
    cy.get("input").should("have.length", 4)
  })

  it("all inputs start empty", () => {
    cy.mount(<InputOTPClient groupLabel="OTP" />)
  ...
  it("has no accessibility violations", () => {
    cy.mount(<InputOTPClient groupLabel="Enter verification code" />)
  ...
  it("accepts digits for each cell individually", () => {
    cy.mount(<InputOTPClient groupLabel="OTP" length={4} />)
