...
- On desktop the calendar dialog renders via a portal with `position: fixed` and repositions on scroll/resize
- Pass `dialogStyle` (`CSSProperties`) to override the calendar dialog's inline styles (e.g. custom width or offset)
...
| -------------------------------- | ------------------------- | -------------------------- |
| `--pk-dp-trigger-bg`             | `var(--pk-color-surface)` | Trigger background         |
...
| `--pk-dp-dialog-radius`          | `var(--pk-radius-lg)`     | Calendar radius            |
| `--pk-dp-dialog-shadow`          | `var(--pk-shadow-lg)`     | Calendar shadow            |
| `--pk-dp-day-today-ring`         | `var(--pk-color-brand)`   | Today ring colour          |
| `--pk-dp-gradient-from`          | `var(--pk-color-brand)`   | Selection gradient start   |
| `--pk-dp-gradient-to`            | `var(--pk-color-accent)`  | Selection gradient end     |
