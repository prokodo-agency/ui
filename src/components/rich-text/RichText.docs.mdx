...
- `color` controls icon and link accent colouring within the rendered markup (`"primary"` | `"secondary"` | `"success"` | `"info"` | `"warning"` | `"error"`)
- When `animated` is `true`, inline text nodes are wrapped in `<AnimatedText>`
- `linkPolicy` controls how external links are sanitised: `"trusted"` (no `rel` added) or `"ugc"` (`rel="ugc nofollow"`)
...
| `--pk-radius-md`            | Inline image border-radius                    |
| `--pk-space-xs`             | List item bottom padding                      |
