...
- `doneLabel` — custom label for the multi-select "Done" button (defaults to `"Done"`)
- `closeAriaLabel` — accessible label for the close button in the mobile sheet (defaults to `"Close"`)
...
| `--pk-select-radius`         | `var(--pk-radius-pill)`        | Corner radius         |
| `--pk-select-shadow`         | `var(--pk-shadow-sm)`          | Trigger shadow        |
| `--pk-select-listbox-shadow` | `var(--pk-shadow-lg)`          | Dropdown shadow       |
...
| `--pk-select-item-hover-bg`  | `var(--pk-palette-primary-50)` | Item hover background |
