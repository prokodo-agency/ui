...
function SliderClient(props: SliderProps) {
  ...
  const snapToStep = useCallback((v: number) =>
    clamp(step > 0 ? Math.round((v - min) / step) * step + min : v),
  [clamp, step, min])

  const snapToMarks = useCallback((v: number) => {
    const c = clamp(v)
  ...
  const quantize = useCallback((v: number) => {
    switch (snap) {
      case "step":  return snapToStep(v)
      case "marks": return snapToMarks(v)
  ...
  const initial = useMemo(() => {
    const numeric =
      typeof controlledValue === "number" ? controlledValue :
      typeof controlledValue === "string" ? Number(controlledValue) : min
  ...
  useEffect(() => {
    if (controlledValue !== undefined) {
      const n = typeof controlledValue === "number"
        ? controlledValue
