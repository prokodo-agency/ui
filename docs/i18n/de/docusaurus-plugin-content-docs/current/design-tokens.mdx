...
Token-Definitionen einmalig im App-Root importieren:
...
:::tip
`theme-tokens.css` enthält **nur** die `--pk-*` Custom Properties — keine Komponenten-Styles. So bleibt dein Critical CSS Pfad klein. Komponenten-Styles werden pro Komponente über eigene CSS-Dateien geladen (z. B. `@prokodo/ui/button.css`). Siehe [Installation](/de/docs/getting-started/installation) für Details.
:::
